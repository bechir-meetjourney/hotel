...
export default function EditInvoice({ invoice, tenants }: Props) {
    const { t } = useT();
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${t('edit', 'Edit')} ${t('invoice', 'Invoice')} #${invoice.invoice_number}`} />
            <div className="mx-auto max-w-4xl p-6">
                <h1 className="mb-6 text-2xl font-bold">{t('edit_invoice', 'Edit Invoice')}: #{invoice.invoice_number}</h1>
                        ...
                        <CardContent>
                            <div className="grid gap-4 sm:grid-cols-2">
                                <Field label={t('invoice_number', 'Invoice Number')}>
                                    <Input value={invoice.invoice_number} readOnly disabled className="vuexy-input bg-muted" />
                                </Field>
